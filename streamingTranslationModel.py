...
from monotonic_decoder import monotonic_decoder
...
class SeamlessM4Tv2ConformerAdapterLayer(nn.Module):
    def __init__(self):
        ...
        self.ffn = SeamlessM4Tv2ConformerFeedForward(dropout=0.1)
...
class SeamlessM4Tv2SpeechEncoder(nn.Module):
    def __init__(self):
        super().__init__()
        # self.feature_projection = AutoFeatureExtractor.from_pretrained("facebook/w2v-bert-2.0") # from huggingface
        self.encoder = Wav2Vec2BertModel.from_pretrained("facebook/w2v-bert-2.0") # from huggingface
        self.intermediate_ffn = SeamlessM4Tv2ConformerFeedForward(dropout=0.0)
        self.adapter = SeamlessM4Tv2ConformerAdapter()
        self.inner_layer_norm = nn.LayerNorm(1024)
        self.final_projection = nn.Linear(1024, 512)
    def forward(
        self,
        inputs,
        attention_mask,
        ...
        output = self.final_projection(hidden_states)
        return output, attention_mask_adapter
...
class StreamingTranslationModel(nn.Module):
    def __init__(self):
        super().__init__()
        # self.text_encoder = get_text_encoder()
        self.text_decoder = monotonic_decoder()
        self.lm_head = torch.nn.Linear(512, 65001, bias=True)
        self.speech_encoder = SeamlessM4Tv2SpeechEncoder()
        
        self.pretrained_model_path = "pretrained_model/model.safetensors"
        self.load_weight_from_pretrained(self.pretrained_model_path)
    
    def load_weight_from_pretrained(self, pretrained_model_path):
        lm_head_weight = {}
        speech_encoder_weight = {}
        decoder_weight = {}
        with safe_open(pretrained_model_path, framework="pt", device='cpu') as f:
            for k in f.keys():
                if(k.split('.')[0] == 'lm_head'):
                    lm_head_weight[k.split('.')[-1]] = f.get_tensor(k).squeeze(0)
                if(k.split('.')[0] == 'speech_encoder'):
                    speech_encoder_weight[k[15:]] = f.get_tensor(k).squeeze(0)
                if(k.split('.')[0] == 'text_decoder'):
                    decoder_weight[k[13:]] = f.get_tensor(k).squeeze(0)
        self.lm_head.load_state_dict(lm_head_weight)
        # self.encoder.load_state_dict(text_encoder)
        self.speech_encoder.load_state_dict(speech_encoder_weight)
        new_model_dict = self.text_decoder.state_dict()
        pretrained_weights = { k:v for k , v in decoder_weight.items() if k in new_model_dict}
        new_model_dict.update(pretrained_weights)
        self.text_decoder.load_state_dict(new_model_dict)
        

    def forward(self, speech_input, speech_attention_mask, decoder_input_ids, decoder_attention_mask):
        ### encoder
        speech_encoder_output, speech_encoder_attention_mask = self.speech_encoder(inputs=speech_input, attention_mask=speech_attention_mask)
        ### for pooling ###
        speech_encoder_attention_mask = keys_pooling(speech_encoder_attention_mask.to(torch.float))
        temp = []
        for seq in speech_encoder_attention_mask:
            temp.append(torch.IntTensor([1 if i>0 else 0 for i in seq]).unsqueeze(0))
        speech_encoder_attention_mask = torch.cat(temp, dim=0).to(torch.device('cuda'))
        ###
        ### decoder
        speech_decoder_output, p_choose = self.text_decoder(input_ids=decoder_input_ids, attention_mask=decoder_attention_mask, encoder_hidden_states=speech_encoder_output, encoder_attention_mask=speech_encoder_attention_mask)
        speech_decoder_output = speech_decoder_output.last_hidden_state

        speech_logits = self.lm_head(speech_decoder_output)
        return speech_logits, speech_encoder_attention_mask, p_choose
    ...
    #     return self.speech_encoder(inputs=speech_input, attention_mask=attention_mask)
